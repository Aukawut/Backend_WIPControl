  ...
  async GetMachine(req, res) {
    try {
      ...
      const result = await pool
        ...
        .input("name", sql.NVarChar, name)
        .query(
          `SELECT * FROM [dbo].[tbl_cmachine] WHERE [status] = 'USE' ORDER BY mach_name`
        );
  ...
  async GetMachineMaster(req, res) {
    try {
      ...
      const result = await pool
        ...
        .input("name", sql.NVarChar, name)
        .query(
          `SELECT * FROM [dbo].[TBL_MACHINE]  WHERE [ACTIVEFLG] = '1' ORDER BY [MCHCD] ASC`
        );
  ...
  async GetAVP2Machine(req, res) {
    try {
      ...
      const result = await pool
        .request()
        .query(
          `SELECT * FROM [PRD_WIPCONTROL].[dbo].[TBL_MACHINE_AVP2] ORDER BY Id ASC`
        );
